    ...
    public CommandLineRunner initializeUrnaState() {
        return args ->{
            ...
            if (cidadaoRepository.count() == 0) {
                // 1. Seed de Cargos (conforme especificação: 1 Presidente e 2 Deputados Federais)
                Cargo pres = cargoRepository.save(Cargo.builder().nome("Presidente").limiteVotos(1).build());
                Cargo depFed = cargoRepository.save(Cargo.builder().nome("Deputado Federal").limiteVotos(2).build());
                ...
                Cidadao c9 = cidadaoRepository.save(Cidadao.builder().cpf("99999999999").nome("Neymar Jr").build());
